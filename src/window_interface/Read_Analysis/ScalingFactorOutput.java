...
public class ScalingFactorOutput extends JFrame {
	...
	public ScalingFactorOutput(ArrayList<File> b, File bl, File c, String out_path, boolean out, int scale, int win, double min) {
...
// 		if(OUTPUTSTATUS) { OUT = new PrintStream(new File(OUTFILE)); }
		...
		for(int z = 0; z < BAMFiles.size(); z++) {
			...
			ScalingFactor script_obj = new ScalingFactor(SAMPLE, BLACKLISTFile, CONTROL, BASENAME, OUTPUTSTATUS, scaleType, windowSize, minFraction);
			script_obj.run();
