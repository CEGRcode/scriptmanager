			...
			try {
				if (chckbxFilterByMinimum.isSelected() && Integer.parseInt(txtMin.getText()) < 0) {
					...
					for (int x = 0; x < BAMFiles.size(); x++) {
						// Initialize LogItem
						String command = BAMtoBEDCLI.getCLIcommand(BAMFiles.get(x), OUT_DIR, STRAND, PAIR, MIN, MAX);
						LogItem new_li = new LogItem(command);
						firePropertyChange("log", old_li, new_li);
						BAMtoBEDOutput convert = new BAMtoBEDOutput(BAMFiles.get(x), OUT_DIR, STRAND, PAIR, MIN, MAX);
