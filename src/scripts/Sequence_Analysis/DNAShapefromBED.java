...
public class DNAShapefromBED {
	...
	public void run() throws IOException, InterruptedException {
		...
		try{
			...
			for(int p=0; p<PS.length; p++){
				if(OUTPUT_TYPE[p] && PS[p]!=null){ PS[p].println(time + "\n" + NAME); }
			}
			openOutputFiles();
			...
			for(int y = 0; y < BED_Coord.size(); y++) {
				try {
					...
					for(int p=0; p<PS.length; p++){
						if(OUTPUT_TYPE[p] && PS[p]!=null){ PS[p].println( "INVALID COORDINATE: " + BED_Coord.get(y).toString()); }
					}
				}
			}
			...
			if(OUTPUT_TYPE[0]){
				...
				for(int z = 0; z < AVG_MGW.length; z++) {
					...
					if(PS[0]!=null){ PS[0].println(DOMAIN_MGW[z] + "\t" + AVG_MGW[z]); }
				}
			...
			if(OUTPUT_TYPE[1]) {
				...
				for(int z = 0; z < AVG_PropT.length; z++) {
					...
					if(PS[1]!=null){ PS[1].println(DOMAIN_PropT[z] + "\t" + AVG_PropT[z]); }
				}
			...
			if(OUTPUT_TYPE[2]) {
				...
				for(int z = 0; z < AVG_HelT.length; z++) {
					...
					if(PS[2]!=null){ PS[2].println(DOMAIN_HelT[z] + "\t" + AVG_HelT[z]); }
				}
			...
			if(OUTPUT_TYPE[3]) {
				...
				for(int z = 0; z < AVG_Roll.length; z++) {
					...
					if(PS[3]!=null){ PS[3].println(DOMAIN_Roll[z] + "\t" + AVG_Roll[z]); }
				}
	...
	public boolean getFAIstatus(){ return INDEX; }
	
	public Component getChartM(){ return chart_M; }
	public Component getChartP(){ return chart_P; }
	public Component getChartH(){ return chart_H; }
	public Component getChartR(){ return chart_R; }
	
	public double[] getAvg(int shapeType){
		if(shapeType==0){ return AVG_MGW; }
		else if(shapeType==1){ return AVG_PropT; }
		else if(shapeType==2){ return AVG_HelT; }
		else if(shapeType==3){ return AVG_Roll; }
		else { return null; }
	}
    ...
    private void openOutputFiles() {
    	...
    	//Open Output File
    	try{
    		if(OUTPUT_TYPE[0]) { OUT_M = new PrintStream(new File(OUTBASENAME + "_MGW.cdt")); }
			if(OUTPUT_TYPE[1]) { OUT_P = new PrintStream(new File(OUTBASENAME + "_PTwist.cdt")); }
			if(OUTPUT_TYPE[2]) { OUT_H = new PrintStream(new File(OUTBASENAME + "_HTwist.cdt")); }
			if(OUTPUT_TYPE[3]) { OUT_R = new PrintStream(new File(OUTBASENAME + "_Roll.cdt")); }
