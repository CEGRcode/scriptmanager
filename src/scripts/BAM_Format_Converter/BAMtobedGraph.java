...
public class BAMtobedGraph {
	...
	public void run() throws IOException, InterruptedException {
		...
		if(f.exists() && !f.isDirectory()) {
			...
			//Print Header
			if(STRAND <= 2) {
				OUTF.println("track type=bedGraph name=\"" + BAM.getName() + "\" description=\"" + getTimeStamp() + ";BedGraph format\" visibility=full color=0,0,255 altColor=255,0,0 priority=20");
				OUTR.println("track type=bedGraph name=\"" + BAM.getName() + "\" description=\"" + getTimeStamp() + ";BedGraph format\" visibility=full color=0,0,255 altColor=255,0,0 priority=20");
			} else {
				OUTF.println("track type=bedGraph name=\"" + BAM.getName() + "\" description=\"" + getTimeStamp() + ";BedGraph format\" visibility=full color=0,0,0 priority=20");
			}
	...
	public void addTag(SAMRecord sr) {
		...
		if(sr.getReadNegativeStrandFlag()) { recordStart = sr.getUnclippedEnd() - 1;} //SAM to BED -1 convert
	...
	public void dumpExcess(String chrom) {
		int trim = 9000;
		while(trim > 0) {
			if(F_OCC.get(0).intValue() != 0) { OUTF.println(chrom + "\t" + BP.get(0).intValue() + "\t" + BP.get(0).intValue() + "\t" + F_OCC.get(0).intValue()); }
	...
	public void dumpMidExcess(String chrom) {
		...
		while(trim > 0) {
			if(M_OCC.get(0).intValue() != 0) { OUTF.println(chrom + "\t" + BP.get(0).intValue() + "\t" + BP.get(0).intValue() + "\t" + M_OCC.get(0).intValue()); }
			BP.remove(0);
			M_OCC.remove(0);
			trim--;
		}
...
// 			System.out.println("Processing: " + seq.getSequenceName());
			...
			for(int z = 0; z < BP.size(); z++) {
				if(F_OCC.get(z).intValue() != 0) { OUTF.println(seq.getSequenceName() + "\t" + BP.get(z).intValue() + "\t" + BP.get(z).intValue() + "\t" + F_OCC.get(z).intValue()); }
				if(R_OCC.get(z).intValue() != 0) {
					int REV = R_OCC.get(z).intValue() * -1;
					OUTR.println(seq.getSequenceName() + "\t" + BP.get(z).intValue() + "\t" + BP.get(z).intValue() + "\t" + REV);
				}
			}
		}
		bai.close();
...
// 			System.out.println("Processing: " + seq.getSequenceName());
			...
			iter.close();
			for(int z = 0; z < BP.size(); z++) {
				if(M_OCC.get(z).intValue() != 0) { OUTF.println(seq.getSequenceName() + "\t" + BP.get(z).intValue() + "\t" + BP.get(z).intValue() + "\t" + M_OCC.get(z).intValue()); }
			}
		}
		bai.close();
	}
