...
// 		System.err.println("OUT="+OUT);
		...
		try{
			SortGFF.sortGFFbyCDT(OUT, gffFile, cdtReference, index[0], index[1]);
	...
	private Integer validateInput(){
		...
		if( output==null){     //not spescified
			OUT = gffFile.getName().substring(0,gffFile.getName().length() - 4) + "_SORT";
		}else{
			try{
				OUT = output.getCanonicalPath();    //specify directory without filename
				if( output.isDirectory() ){
					OUT = OUT + File.separator + gffFile.getName().substring(0,gffFile.getName().length() - 4) + "_SORT";
				}
