...
import scripts.Cluster_Scripts.CorrelationMatrix;
import util.FileSelection;

@SuppressWarnings("serial")
public class CorrelationMatrixWindow extends JFrame implements ActionListener, PropertyChangeListener {
        ...
        public Void doInBackground() throws IOException {
        	if(TABFiles.size() < 1) {
        		...
        		for(int x = 0; x < TABFiles.size(); x++) {
        			CorrelationMatrix matrix = new CorrelationMatrix(TABFiles.get(x), OUTPUT_PATH, comboBox.getSelectedIndex(), rdbtnCorrelateColumns.isSelected());
            		matrix.run();
	...
	public CorrelationMatrixWindow() {
		setTitle("Generate Correlation Matrix");
