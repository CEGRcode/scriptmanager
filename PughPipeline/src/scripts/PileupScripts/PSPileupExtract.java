...
public class PSPileupExtract implements Runnable{
	...
	public PSPileupExtract(PileupParameters p, File b, Vector<BEDCoord> i, int current, int length) {
		param = p;
		BAM = b;
		INPUT = i;
		index = current;
		...
		while (iter.hasNext()) {
			...
			if(sr.getReadPairedFlag()) {
				if(sr.getProperPairFlag()) {
					if(((sr.getFirstOfPairFlag() && param.getRead() == 0) || (!sr.getFirstOfPairFlag() && param.getRead() == 1) || param.getRead() == 2) && sr.getReadString().charAt(0) == 'T') {
			...
			} else if((param.getRead() == 0 || param.getRead() == 2) && sr.getReadString().charAt(0) == 'T') {
