...
import scripts.PileupScripts.PSPileupExtract;
import scripts.PileupScripts.TransformArray;
import util.JTVOutput;

@SuppressWarnings("serial")
public class PSTagPileup extends JFrame {
	...
	public PSTagPileup(Vector<BEDCoord> in, Vector<File> ba, PileupParameters param) {
		...
		for(int z = 0; z < BAMFiles.size(); z++) {
			...
			//if(f.exists() && !f.isDirectory()) {
				...
				for(int x = 0; x < CPU; x++) {
					...
					PSPileupExtract extract = new PSPileupExtract(PARAM, BAM, INPUT, currentindex, subset);
